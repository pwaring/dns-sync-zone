...
def validate_zone_record(zone_record):
    ...
    if skip_zone_record(zone_record):
        ...
        if len(zone_record_parts) >= 5:
            ...
            if record_command in valid_commands and record_ttl.isdigit() and (record_type in valid_types_basic or record_type in valid_types_extra):
                if len(zone_record_parts) == 5:
                    # Looks like a standard record type
                    record_data = zone_record_parts[4]

                    if record_type in valid_types_basic or record_type == 'TXT':
                        return True
